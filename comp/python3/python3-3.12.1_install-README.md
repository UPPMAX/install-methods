...
Structure creating script (makeroom_python3_3.12.1.sh) moved to /sw/comp/python3/makeroom_3.12.1.sh

Starting with version 3.9.5, abandon the ${VERSION}_${CLUSTER} prefix structure
for the usual $VERSION/$CLUSTER.
...
that matches what we have done for the python3 module.  This is simply legacy
stuff, that this python3 module is structured this way.


LOG
---

    makeroom.sh "-f" "-t" "python3" "-v" "3.12.1" "-c" "comp" "-w" "https://python.org/" "-l" "PSF License Agreement" "-L" "https://docs.python.org/3/license.html" "-d" "Python programming language plus a handful of widely used packages ; this module does not conflict with python/x.y.z modules"
    ./makeroom_python3_3.12.1.sh

    source /sw/comp/python3/SOURCEME_python3_3.12.1
    cd $SRCDIR
...
Now install the module. This will be adjusted later.

    cd $TOOLDIR
    ./python3-3.12.1_post-install.sh
...
We do not install these for this module, unlike for python/3.12.1.
