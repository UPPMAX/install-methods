...
Structure creating script (makeroom_julia_1.10.10_LTS.sh) moved to /sw/comp/julia/makeroom_1.10.10_LTS.sh
...
---
    ...
    ./makeroom_julia_1.10.10_LTS.sh
    source /sw/comp/julia/SOURCEME_julia_1.10.10_LTS && cd $SRCDIR
    wget https://julialang-s3.julialang.org/bin/linux/x64/1.10/julia-1.10.10-linux-x86_64.tar.gz
    tar xzf julia-1.10.10-linux-x86_64.tar.gz -C $PREFIX --strip-components=1
...
# Fix mf file to correct versions
    prepend-path    JULIA_LOAD_PATH     $modroot/lib/glob_pkg/environments/v1.10
    setenv    JULIA_PROJECT       $userhome/.julia/environments/v1.10


#    start julia session from module
    empty!(DEPOT_PATH)
    using Pkg
    push!(DEPOT_PATH,"/sw/comp/julia/1.10.10_LTS/rackham/lib/glob_pkg") 
    Pkg.activate(DEPOT_PATH[1]*"/environments/v1.10");
