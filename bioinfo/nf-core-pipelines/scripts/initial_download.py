...
# set the path before running the script
module_path = os.environ["PREFIX"]
#os.environ["NXF_SINGULARITY_CACHEDIR"] = "/sw/bioinfo/nf-core-pipelines/singularity_cache_dir"
#os.environ["NXF_HOME"] = "/sw/bioinfo/nf-core-pipelines/nxf_home"


# loop through each pipeline workflow
for wf in wfs.remote_workflows[:1]:
    print(wf.full_name)
    ...
    for release in wf.releases:
        ...
        shutil.rmtree("/sw/bioinfo/nf-core-pipelines/nxf_home")
        os.mkdir("/sw/bioinfo/nf-core-pipelines/nxf_home")
