...
Structure creating script (makeroom_R_4.5.1.sh) moved to /sw/apps/R/makeroom_4.5.1.sh

Need to update the gcc and java versions. The hard stuff will have to wait until R_packages/4.5.1.

LOG
---

    makeroom.sh "-f" "-t" "R" "-v" "4.5.1" "-c" "apps" "-w" "https://www.r-project.org" "-d" "free software environment for statistical computing and graphics" "-l" "GPL v2"
    ./makeroom_R_4.5.1.sh


    source /sw/apps/R/SOURCEME_R_4.5.1
...
The mf file 4.5.1.lua needs to load gcc, java, cairo, texlive, texinfo, libcurl, readline, libicu, libdeflate, etc. with the above versions. AND ALSO openblas/0.3.29.

### Do fixup

Refer to mf/4.5.1.lua rather than mf/4.5.1

    cd $TOOLDIR
    vi ./R-4.5.1_post-install.sh
    ./R-4.5.1_post-install.sh
