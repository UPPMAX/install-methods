...
Structure creating script (makeroom_python_ML_packages_3.11.8-cpu.sh) moved to /sw/apps/python_ML_packages/makeroom_3.11.8-cpu.sh
...
```bash
    ...
    makeroom.sh "-f" "-t" "python_ML_packages" "-v" "3.11.8-cpu" "-c" "apps" "-w" "https://pypi.org" "-l" "Various" "-d" "A selection of Python packages for Machine Learning" "-u" "rackham"
    ./makeroom_python_ML_packages_3.11.8-cpu.sh
    
    cd /sw/apps/python_ML_packages
    source /sw/apps/python_ML_packages/SOURCEME_python_ML_packages_3.11.8-cpu

# update the PREFIX
    cd /sw/apps/python_ML_packages/3.11.8-cpu
    mv rackham rackham-centos7
    export PREFIX=/sw/apps/python_ML_packages/3.11.8-cpu/rackham-centos7
    cd $PREFIX
...
# for pip list -v
    ...
    PYTHONUSERBASE=$PREFIX pip3 install --user --upgrade tensorflow-cpu
...
#    PYTHONUSERBASE=$PREFIX pip3 install --user --upgrade keras
    ...
    PYTHONUSERBASE=$PREFIX pip3 install --user --upgrade torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cpu
...
# lock it down
    ...
    cd $TOOLDIR
    vi mf/3.11.8-cpu
    vi python_ML_packages-3.11.8-cpu_install-README.md
    vi python_ML_packages-3.11.8-cpu_post-install.sh
    ./python_ML_packages-3.11.8-cpu_post-install.sh
